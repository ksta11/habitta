  ...
  const applyFilters = async () => {
    try {
      ...
      if (response.success) {
        ...
        console.log(
          `✅ ${filteredData.length} propiedades encontradas con filtros`
        );
      } else {
  ...
  const applyFiltersWithCategory = async (categoryValue: string) => {
    try {
      ...
      if (response.success) {
        ...
        setProperties(filteredData);
        console.log(
          `✅ ${filteredData.length} propiedades encontradas con categoría: ${categoryValue}`
        );
      } else {
        console.log("❌ Error al aplicar filtros:", response.message);
        Alert.alert(
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <HomeHeader
        onNavigateToReviews={() => {
          console.log(
            "🚀 [HomeModule] Función de navegación a reviews ejecutada"
          );
      ...
      >
        ...
        <View className="px-6 pb-24">
          <View className="flex-row items-center justify-between mb-4">
            <Label
              text={`Propiedades (${properties.length})`}
              size="lg"
              weight="semibold"
            />
            <Pressable>
